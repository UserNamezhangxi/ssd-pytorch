...
classes, cls_number = get_classes('model_data/voc_classes.txt')


def convert_annotation(image_id, list_file):
    dataset_path = 'VOCdevkit/VOC2007/Annotations'
...
if __name__ == "__main__":
    ...
    for img_id in image_ids:
        list_file.write(os.path.abspath(os.path.join('VOCdevkit/VOC2007/JPEGImages', img_id + '.jpg')))
        convert_annotation(img_id, list_file)
        list_file.write('\n')
    list_file.close()

    image_ids = open('VOCdevkit/VOC2007/ImageSets/Main/val.txt').read().strip().split()
    list_file = open('2007_val.txt', 'w')
    for img_id in image_ids:
        list_file.write(os.path.abspath(os.path.join('VOCdevkit/VOC2007/JPEGImages', img_id + '.jpg')))
        convert_annotation(img_id, list_file)
        list_file.write('\n')
